...
def asian_option_prices(paths, r, t, K):
    c = np.zeros(len(K))
    p = np.zeros(len(K))

    for i in range(len(K)):
        c[i] = round(np.mean(np.maximum(np.mean(paths, axis=0) - K[i], 0)) / np.exp(r * t), 2)
        p[i] = round(np.mean(np.maximum(K[i] - np.mean(paths, axis=0), 0)) / np.exp(r * t), 2)
...
print(euro_option_prices(P, 0.01, 1, [95,96,97]))
print(asian_option_prices(P, 0.01, 1, [95,96,97]))
